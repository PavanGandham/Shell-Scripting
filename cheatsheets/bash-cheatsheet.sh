...
@vik-y
Author
vik-y commented on Mar 29, 2018
() # Execute in Subshell
$$ # Process id 
$n # nth command line argument 
$? # return value of last command we run
To try and understand

[] 
@vik-y
Author
vik-y commented on May 28, 2018
find . -type f -name name\*
Search in current directories as well as subdirectories for all files starting with "name"

If you want to search for file as well as folders
find -name name\*

To delete all the files which you obtained
find . -type f -name name\* -exec rm {} \;

Always check the list of files before using the exec command

@vik-y
Author
vik-y commented on Jun 25, 2019
Find 5 largest pdf files in current directory :

sudo find . -type f -name "*.pdf" -exec ls -l {} \; | awk '{print $5 " " $9}' | sort -nr -k1 | head -n5

@vik-y
Author
vik-y commented on May 3
grep -A10 # Print 10 lines after the matched pattern
grep -B10 # Print 10 lines before the matched pattern
grep -C10 # Print 10 lines before and after the matched pattern

grep 'vikas|deepak' # Match vikas or deepak
grep -v # invert matching # negation

@vik-y
Author
vik-y commented on May 3
Discover your dhcp server details

~ ❯❯❯ sudo nmap --script broadcast-dhcp-discover

Starting Nmap 7.60 ( https://nmap.org ) at 2020-03-15 11:04 IST
...
|     Router: 192.168.1.1
|     Domain Name Server: 176.103.130.130, 176.103.130.130, 176.103.130.131
|_    Domain Name: domain.name
WARNING: No targets were specified, so 0 hosts scanned.
Nmap done: 0 IP addresses (0 hosts up) scanned in 1.94 seconds